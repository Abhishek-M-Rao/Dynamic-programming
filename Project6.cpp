...
int minRec1(int x[], int n) {
    //need a base case and need the logic that finds the minimum between at least 1
    //x, n-1
    if(n == 1){
        return x[0];
    }
...
double sqrtIt(double x, double low_guess, double high_guess) {
    //actually it seems like I do need the high low guess for both
    //different function signature - define another recursive function in the function
    double avgGuess = (low_guess + high_guess)/2;
    while (((avgGuess - (x/avgGuess)) > 0.00000000000001) || (avgGuess - (x/avgGuess)) < -0.00000000000001){
            avgGuess = (avgGuess + (x/avgGuess))/2;
    }
    return avgGuess;
}
...
double sqrtRec(double x, double low_guess, double high_guess) {
    ...
    if ((guess - (x/guess) < 0.000000000000001) && (guess - (x/guess) > -0.000000000000001)) {
...
int strCompare(char* str1, char* str2) {
    ...
    //otherwise increment both and call function again
...
int solveMazeRec(int row, int col) {
    int down_track = 0, right_track = 0, left_track = 0, up_track = 0;

    //I want the base case to be that I have reached the end of the maze
    if(row == (MATRIX_SIZE - 1) && maze[row][col] == 0){
        maze[row][col] = 2;
        return 1; //same as return true
    }
    maze[row][col] = 2; //set up breadcrumb where you are at
    if(row < (MATRIX_SIZE - 1) && row >= 0){
        if(maze[row+1][col] == 0){
            down_track = solveMazeRec(row+1, col);
        }
        if(down_track == 1){
            return down_track;
        }
    ...
    if(col < MATRIX_SIZE && col >= 0){
        if(maze[row][col+1] == 0){
            right_track = solveMazeRec(row, col+1);
        }
        if(right_track == 1){
            return right_track;
        }
    ...
    if(col < MATRIX_SIZE && col > 0){
        if(maze[row][col-1] == 0){
            left_track = solveMazeRec(row, col-1);
        }
        if(left_track == 1){
            return left_track;
        }
    ...
    if(row < MATRIX_SIZE && row > 0){
        if(maze[row-1][col] == 0){
            up_track = solveMazeRec(row-1, col);
        }
        if(up_track == 1){
            return up_track;
        }
    }
        maze[row][col] = 0; //if you can't go anywhere, set where you are as 0 and return false
        return 0;
}
...
Martian LeastCoins(Martian mar1, Martian mar2, Martian mar3);
Martian LeastCoins(Martian mar1, Martian mar2, Martian mar3){
    int s1 = mar1.pennies + mar1.nicks + mar1.dodeks;
    int s2 = mar2.pennies + mar2.nicks + mar2.dodeks;
    int s3 = mar3.pennies + mar3.nicks + mar3.dodeks;
    if(s1 != 0 && s2 != 0 && s3 != 0){
        if(s1 <= s2 && s1 <= s3){return mar1;}
        if(s2 <= s1 && s2 <= s3){return mar2;}
        else{return mar3;}
    }
    if(s1 == 0){
        if(s2 == 0){return mar3;}
        if(s3 == 0){return mar2;}
        if(s2 <= s3){return mar2;}
        else{return mar3;}
    }
    if(s2 == 0){
        if(s3 == 0){return mar1;}
        if(s1 <= s3){return mar1;}
        else{return mar3;}
    }
    if(s1 < s2){return mar1;}
    else{return mar2;}
}
...
Martian change(int cents) {
    Martian Martian1, Martian2, Martian3;
    Martian1 = {0,0,0};
    Martian2 = {0,0,0};
    Martian3 = {0,0,0};
    if(cents < 0){
    ...
    if((cents - 12) >= 0){
        Martian1 = change(cents - 12);
        Martian1.dodeks++;
    }
    if((cents - 5) >= 0){
        Martian2 = change(cents - 5);
        Martian2.nicks++;
    }
    if((cents - 1) >= 0){
            Martian3 = change(cents - 1);
            Martian3.pennies++;
    }
    return LeastCoins(Martian1, Martian2, Martian3);
}
...
Martian change(int cents, int nick_val, int dodek_val) {
    ...
    if((cents - dodek_val) >= 0){
        Martian1 = change(cents - dodek_val, nick_val, dodek_val);
        Martian1.dodeks++;
    }
    if((cents - nick_val) >= 0){
        Martian2 = change(cents - nick_val, nick_val, dodek_val);
        Martian2.nicks++;
    }
    if((cents - 1) >= 0){
            Martian3 = change(cents - 1, nick_val, dodek_val);
            Martian3.pennies++;
    }
    return LeastCoins(Martian1, Martian2, Martian3);
}
